...
// Hungry JS Atom/RSS parser. Downloads and extract useful info from a
...
define(['underscore'], function(_) {
    ...
    // NOTE: This will NOT download any feed that violates the JS
    // same-origin policy unless the host has CORS or this is a packaged,
    // privileged Firefox OS app with the `systemXHR` permission.
    function download(url, callback) {
        var request = new window.XMLHttpRequest({mozSystem: true});

        request.open('GET', url, true);

        // TODO: Firefox OS seems to throw an NS_FILE_NOT_FOUND error on
        // XMLHttpRequest 404s, but maybe that's just for same-origin requests?
        try {
    ...
    // Parse a feed (either actual feed text or a parsed XML object) and
    // return a subset of the data as a useful object full of feed data.
